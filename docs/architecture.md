...
The Connections dashboard resolves active Supabase connection labels from the existing project catalog. Grants covering multiple projects show their names together; unavailable or empty catalogs fall back to the saved connection label. This also applies to previously connected accounts without a migration or reconnection.
