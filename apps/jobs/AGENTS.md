...
Implemented today: this is a SINGLE PROJECT child (data pipeline / operations runtime) within the root pnpm WORKSPACE; it has no nested source projects. All commands run from repository root unless explicitly stated. Follow [root AGENTS.md](../../AGENTS.md), preserve unrelated changes, use YAGNI and keep files around 500 lines or less. Browser tests require explicit task authorization and isolated identities/_test databases. Report source changes, verification, deployment and live acceptance separately.
