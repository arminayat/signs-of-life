# Jobs runtime folder structure

## Implemented today

- src/node.ts: Node direct-claim worker loop.
- src/cloudflare.ts: Cron dispatcher and Queue consumer.
...
- package.json: private workspace marker; AGENTS.md/docs/agents/: memory.
  Local tests/, migrations/, dist/ source output and provider collectors are not present here. Tests, Dockerfile, compose.yaml and command scripts live at root.
