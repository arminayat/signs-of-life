...
Root drizzle.config.ts reads application, identity and monitoring schema modules and generates migrations/meta. pnpm db:generate generates SQL; pnpm db:migrate applies it from the repository root. Root Vitest runs database suites serially; no child-specific CI/deploy exists.
